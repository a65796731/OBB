...
public class HapticEffect : MonoBehaviour
{
	...
	public Vector3 Position = Vector3.up * -5;
	...
	void Start()
	{
		...
		sphere = GameObject.Find("Scene/HapticDeviceWithGrabber/Grabber/Sphere").transform;
	}
	public static bool IsDrag = false;
	public static bool beginDrag = false;
	bool IsCollision;
	...
	void Update()
	{
		...
		{
			List<Vector3> activedParticals=  GameObject.Find("B_1").GetComponent<SSPSoftPart>().activedParticals;
			...
			for (int ii = 0; ii < devices.Length; ii++)
			{
				...
				min = float.MaxValue;
				index = -1;
				for (int i = 0; i < activedParticals.Count; i++)
				{
					float dis = (activedParticals[i] - StylusPos).magnitude;
					if (min > dis)
					{
						min = dis;
						index = i;
						CollisionPoint = activedParticals[i];
					}
				}
				delta[ii] = min;
				if (min <= 0.1f)
		...
		if (IsDrag)
		{
			for (int ii = 0; ii < devices.Length; ii++)
			{
				...
				Vector3 StylusPos = device.stylusPositionWorld; //World Coordinates
				Debug.Log((CollisionPoint - StylusPos).magnitude);
				if ((CollisionPoint - StylusPos).magnitude > 1.5f)
				{
					beginDrag = false;
					HapticPlugin.effects_stopEffect(device.configName, ID);
				}
	...
	{
		if (sphere != null)
			Gizmos.DrawLine(sphere.position, sphere.position + sphere.forward * -1);
